...
for i in range(first_filenum, last_filenum+1):
    ...
    # Define b vector
    # FILL IN CODE HERE
    b = (nu*div(grad_velocity)) - udot - grad(velocity)*velocity

    # Set up forms and solve
    # FILL IN CODE HERE
    left = dot(grad(w),grad(P))*dx
    right = dot(grad(w),b)*dx
    # p = Function(Q)
    solve(left == right, sol , bc)
    p = sol
    ...
    p.vector().set_local(p.vector().get_local() - np.amin(p.vector().get_local()))
